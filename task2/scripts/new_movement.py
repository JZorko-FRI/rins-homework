...
class MovementNode:
    def __init__(self):
        ...
        self.arm_controller_pub = rospy.Publisher(
            "/task2/arm_controller/command", String, queue_size=10
        )
        ...
        self.cancel_sub = rospy.Subscriber(
            "task2/command", String, self.command_callback
        )
        self.bridge = CvBridge()

        self.circle_sub = rospy.Subscriber(
            "/task2/all_circles", Pose, self.circle_callback
        )
        self.cylinder_sub = rospy.Subscriber(
            "/task2/all_cylinders", String, self.cylinder_callback
        )

        self.cylinder_one_sub = rospy.Subscriber(
            "/task2/cylinder_pose", Marker, self.cylinder_one_callback
        )
